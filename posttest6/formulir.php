...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Ambil dan sanitasi data input dari formulir
    ...
    $subscriptionType = mysqli_real_escape_string($conn, $_POST['subscription_type'] ?? '');
    ...
    if (isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
        ...
        // Ganti nama file dengan timestamp dan simpan di direktori uploads
    ...
    $emailCheckQuery = "SELECT * FROM `join` WHERE email = ?";
    ...
    if ($result->num_rows > 0) {
        echo "<script>alert('Email sudah terdaftar. Silakan gunakan email lain.'); window.location.href = 'formulir.php';</script>";
        exit();
    }

    // Simpan data ke tabel join
    $sql = "INSERT INTO `join` (full_name, email, Age, City, state, country, subscription_type, password, photo) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);

    // Bind parameter dan eksekusi query
    $stmt->bind_param("ssissssss", $fullName, $email, $age, $city, $state, $country, $subscriptionType, $hashedPassword, $photoPath);
    
    if ($stmt->execute()) {
        header("Location: tampilkan_data.php"); // Redirect setelah sukses
        exit; 
    } else {
...
<body>
    <header>
        ...
        <main>
            <section class="content">
                ...
                <p>"The Lan National Museum of Natural History is a place of awe and wonder, preserving an irreplaceable
                    record of Earth’s ever-changing physical, biological, and cultural diversity. Join our community
                    today and together we will fuel curiosity and inspire the next generation of knowledge seekers
                    through active exploration and discovery."</p>
            </section>
        </main>
        ...
        <div class="form-container">
            ...
            <p>Sign up to receive email communication from the Museum, and we'll send you the latest news about
                scientific discovery, details on exciting exhibit openings, and other programming that occurs only at
                your National Museum of Natural History.</p>
